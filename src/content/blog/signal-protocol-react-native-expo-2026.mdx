...
If you have no relay yet, use `mockRelay()` from `@open-e2ee/signal-protocol-sdk/remote/relay/mock`. The same code then runs entirely in memory.
...
The SDK is `@open-e2ee/signal-protocol-sdk` on the `0.1.x` alpha line. Public APIs and persisted formats may change before 1.0. No third-party security audit exists today. OpenE2EE plans an independent review.
