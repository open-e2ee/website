...
description: The official JavaScript library is archived. Its replacement is a Node native add-on, and the community port has an open trust-check bug.
...
Use the same client composition on both devices. This example reaches the OpenE2EE Signal Protocol Relay. The `getIdentityAssertion` callback is your own function: it returns the signed identity assertion from your identity provider.
...
const signal = await createHostedSignalProtocolClient({
  hosted: { relayUrl, getIdentityAssertion: signIn },
...
On first run, the client generates the identity keys and publishes the public material to the relay during initial synchronization. The first send establishes a session.
...
If you have no Relay project yet, use `createSignalProtocolClient()` from the package root with `identity: { userId }` and `inMemoryRelay()` from `@open-e2ee/signal-protocol-sdk/remote/relay/memory` as the `relay` adapter. The same conversation then runs entirely in memory, against real protocol and real cryptography.

## Where the keys live

An [`r/reactnative` thread](https://www.reddit.com/r/reactnative/comments/1bsqbnr/signal_protocol_on_rn/) asked where an Expo app should store keys. It also asked about key lifetime and preventing leaks to other apps or the relay. The thread had no answer during the 24 July 2026 review.

<figure class="diagram">
  <ExpoKeyMap />
  <figcaption>
    Private key material stays inside the device. Only the public prekey bundle leaves it. A solid
    notch marks private material, and an open notch marks the bundle that travels.
  </figcaption>
</figure>
...
Do not upload or synchronize the SQLite file with the relay. Before a generic "device backup" feature includes this file, read [Recovery, backup, and migration](https://docs.open-e2ee.dev/build/recovery-backup-migration).
